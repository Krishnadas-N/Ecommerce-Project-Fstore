...
const updateCouponStatus = async () => {
  try {
    ...
    const activeCoupons = await Coupon.find({
      isActive: true,
      startDate: { $lte: currentDate }, // Check if the current date is after or equal to the start date
      expirationDate: { $lte: currentDate }, // Check if the current date is after or equal to the expiration date
    });
    ...
    await Coupon.updateMany(
      { _id: { $in: activeCoupons.map((coupon) => coupon._id) } },
      { isActive: true }
    );
...
//https://chat.openai.com/share/2a982737-e34b-42a8-a2ae-9b532ded7568
